  ...
  return (
    <>
      ...
      {!isOpen && (
        <button
          onClick={onToggle}
          className="fixed bottom-5 right-5 z-40 flex items-center gap-2.5 bg-gradient-to-r from-emerald-700 to-teal-800 hover:from-emerald-600 hover:to-teal-700 text-white font-extrabold px-4 py-3 sm:px-5 sm:py-3.5 rounded-full shadow-xl hover:shadow-2xl border-2 border-emerald-400/40 transition-all duration-200 hover:scale-105 active:scale-95 group cursor-pointer"
          id="btn-floating-ai-agent"
          title={t.aiAgent.buttonText}
        >
          <div className="w-8 h-8 rounded-full bg-white text-emerald-900 flex items-center justify-center shadow-xs">
      ...
      {isOpen && (
        <div
          className={`fixed z-50 transition-all duration-200 shadow-2xl flex flex-col overflow-hidden bg-white border border-stone-200 ${
            isExpanded
        ...
        >
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="font-extrabold text-sm text-white flex items-center gap-1.5">
                  ...
                  <span className="text-[10px] bg-emerald-500/30 text-emerald-200 px-1.5 py-0.2 rounded-sm font-mono">
                    Agro-AI
                  </span>
                </h3>
          ...
          <div className="bg-emerald-50/80 px-4 py-2 border-b border-emerald-100 flex items-center gap-2 overflow-x-auto text-[10px] text-emerald-900 font-semibold no-scrollbar shrink-0">
            {location && (
              <span className="bg-white px-2 py-0.5 rounded-md border border-emerald-200 shrink-0 flex items-center gap-1">
            ...
            {visionResult && (
              <span className="bg-emerald-100 text-emerald-900 px-2 py-0.5 rounded-md border border-emerald-300 shrink-0 flex items-center gap-1 font-mono text-[9px]">
            ...
            {location?.agroClimate && (
              <span className="bg-emerald-100 text-emerald-800 px-2 py-0.5 rounded-md border border-emerald-300 shrink-0">
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-3.5 bg-stone-50 text-xs">
                ...
                <div className="flex items-center gap-2 mt-1 px-1 text-[10px] text-stone-400">
                  ...
                  {msg.sender === "ai" && (
                    <button
                      ...
                      className="hover:text-emerald-700 transition-colors p-0.5 cursor-pointer"
                      title="Read aloud"
                    >
            ...
            {isLoading && (
              <div className="flex items-center gap-2 text-stone-500 text-xs p-2.5 bg-white rounded-xl border border-stone-200 w-fit">
          ...
          <div className="px-3 pt-2 pb-1 bg-white border-t border-stone-100 flex items-center gap-1.5 overflow-x-auto no-scrollbar shrink-0">
            {dynamicSuggestedQuestions.map((q, idx) => (
              <button
                key={idx}
                onClick={() => handleSendMessage(q)}
                className="text-[11px] font-semibold bg-stone-100 hover:bg-emerald-50 text-stone-700 hover:text-emerald-900 px-2.5 py-1 rounded-full border border-stone-200 hover:border-emerald-300 whitespace-nowrap transition-colors cursor-pointer"
              >
                {q}
              </button>
            ))}
          ...
          <div className="p-3 bg-white border-t border-stone-200 flex items-center gap-2 shrink-0">
            <input
              ...
              className="flex-1 bg-stone-50 border border-stone-300 rounded-xl px-3.5 py-2 text-xs text-stone-900 focus:outline-none focus:ring-2 focus:ring-emerald-500"
            ...
            <button
              ...
              className="p-2.5 rounded-xl bg-emerald-700 hover:bg-emerald-600 disabled:opacity-50 text-white transition-colors cursor-pointer"
              id="btn-send-ai-chat"
            >
